...
ODU tagger Cherenkov simulation - 2024
C. Ayerbe Gayoso - gayoso@jlab.org

Instructions:
- Install ROBAST and set the enviroment variables, according the ROBAST webpage

 https://robast.github.io/index.html
- Run the script as:
   root ODU_cerenkov.C+
or:
root 
root [0] .x ODU_cerenkov.C+
...
// the position of the ENTRANCE of the Winston Cone
// For Mirror alignment, the value MUST be 0, which corresponds to the wall
...
// Photons will 'emulate' the creation of Cherenkov photons in a cone of 12. deg
// Rays will represent muons
bool rayORphotons = 0;
...
// CHOOSE A CASE (IF randomDist = 1, not used) 
...
AOpticsManager* MakeGeometry() {
  ...
  //          x/
  //
  // But (0,0,0) is the bottom of the box, where the mirror stands
  //
  ...
  TGeoTranslation* box_trX2 =
    ...
    new TGeoTranslation("box_trX2", -x_box/2, 0, z_box/2); // this position is wrt the focal point estimated
  // large box wall
  TGeoTranslation* box_trY1 =
    ...
    new TGeoTranslation("box_trY1", 0, y_box/2, z_box/2); // this position is wrt the focal point estimated
  // large box wall
  TGeoTranslation* box_trY2 =
    ...
    new TGeoTranslation("box_trY2", 0, -y_box/2, z_box/2); // this position is wrt the focal point estimated
  // base of the box
  TGeoTranslation* box_trZ1 =
    //    new TGeoTranslation("box_trZ1", 0, 0, dist_T1_FP+2*cm); // this position is wrt the focal point estimated
    new TGeoTranslation("box_trZ1", 0, 0, 0); // this position is wrt the focal point estimated


  
  // All the next lines is to make the walls opaque to the rays. 
  //Remember, the coordinate names correspond to the plates, perpendicular to the corresponding axis

  // no plate to the PMT side of the box, to allow the rays go through
  ...
  // No top plate of the box. We can add it if it is make of vacuum,
  ...
  Double_t dist_ori = 18.73*cm; //I gave unit to each distance to avoid conflicts from previously defined variables
  ...
  //******************************************************************
  // THIS IS THE DESIGN OF THE REAL MIRROR 
  //******************************************************************
  ...
  //******************************************************************
  //THE CASE SELECTION IS MADE AT THE BEGGINING OF THE CODE
  //******************************************************************
  switch(mirror2use)
    {
    case 1:
  ...
  TGeoTranslation* focal_tr =
    ...
    new TGeoTranslation("focal_tr", x_box/2 + foc_sep*cm, 0, 20*cm );
...
// These function are just mathematical functions to generate the Cherenkov rays
...
void ODU_cerenkov()
{
  ...
  Double_t dist_ori = (5 + 11.0 + 3.32)*cm;
  ...
  for (Int_t i = 0; i < kN; i++)
      ...
      if (randomDist)
	{
      ...
      // and Elliptical:
      // the elliptical distribution is an optimized code to produce
      // all the rays within the aceptance of the mirror.
      // Remember the projection of the tilted spherical mirror
      // to the top box is an ellipse
      //******************************************************
	  ...
	  for (Int_t j = 0; j <= focused->GetLast(); j++)
	    {
	      ...
	      hist->Fill(p[1], p[2] -(2.5*cm+20*cm) );// In the Box system, Y(box) is X in the histo, Z(box) is Y in the histo
	    }  // j
